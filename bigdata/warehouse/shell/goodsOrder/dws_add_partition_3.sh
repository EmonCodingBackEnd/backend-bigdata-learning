...
hive -e "
insert overwrite table dws_warehousedb.dws_order_goods_all_info partition(dt='${dt}')  select 
   doi.order_id,
   doi.goods_id,
   doi.goods_amount,
...
from dwd_warehousedb.dwd_order_item as doi
left join dwd_warehousedb.dwd_goods_info as dgi
on doi.goods_id = dgi.goods_id
left join dwd_warehousedb.dwd_category_code as dcc
