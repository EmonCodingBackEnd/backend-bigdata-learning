...
public class TableToDataStreamJava {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 获取table
        Table table = streamTableEnv.from("myTable");

        // 将table转换为DataStream
        // 如果只有新增（追加）操作，可以使用toAppendStream
        DataStream<Row> appStream = streamTableEnv.toAppendStream(table, Row.class);
        appStream
                .map(
                        new MapFunction<Row, Tuple2<Integer, String>>() {
                            @Override
        ...
        DataStream<Tuple2<Boolean, Row>> retStream =
                streamTableEnv.toRetractStream(table, Row.class);
        retStream
                .map(
                        new MapFunction<Tuple2<Boolean, Row>, Tuple3<Boolean, Integer, String>>() {
                            @Override
                            public Tuple3<Boolean, Integer, String> map(Tuple2<Boolean, Row> tup)
                                    throws Exception {
                                Boolean flag = tup.f0;
                                Integer id = Integer.parseInt(tup.f1.getField(0).toString());
                                String name = tup.f1.getField(0).toString();
        ...
        streamEnv.execute(TableToDataStreamJava.class.getSimpleName());
    }
}
